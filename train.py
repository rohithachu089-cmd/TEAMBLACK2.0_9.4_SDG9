...
def load_dataset(data_dir: str, image_size=(224, 224), limit=None):
...
def main():
    parser = argparse.ArgumentParser(description='Train equipment defect classifier (normal/bearing_failure/overheating/connect_disconnection) and export TFLite.')
    parser.add_argument('--data', default='dataset', help='Path to dataset directory with equipment failure subfolders')
    parser.add_argument('--epochs', type=int, default=10)
    parser.add_argument('--fine_tune_epochs', type=int, default=5)
    parser.add_argument('--limit', type=int, default=None, help='Limit images per class for quick testing')
    args = parser.parse_args()

    X, y, classes = load_dataset(args.data, image_size=MODEL_INPUT_SIZE, limit=args.limit)

    # Handle tiny datasets for demo purposes
    print("⚠️ Using full dataset for training (Demo Mode)")
